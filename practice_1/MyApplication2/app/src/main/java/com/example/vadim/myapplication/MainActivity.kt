...
class MainActivity : AppCompatActivity() {
    ...
    var repos = GitHubRepositoryInfo.List()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Thread(Runnable {
            ...
            repos = Gson().fromJson(responseText, GitHubRepositoryInfo.List::class.java)
            android.util.Log.d("Repos", repos.joinToString { it.name })
            System.out.println("[THIS]: " + repos[0].name);

            System.out.println("[2222]");
            repos.forEach{
                i -> countries.add(i.name)
            }
